...
<div id="add-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'"
    ...
    <form id="add-form" method="post">
        <table>
            ...
            <tr>
                <td width="60" align="right">是否可用:</td>
                <td><label>
                    <input type="radio" name="available" value="1"/>可用
                </label>
                    <label>
...
<div id="select-authority-dialog" class="easyui-dialog" data-options="closed:true,iconCls:'icon-save'"
    ...
    <ul id="authority-tree" url="${pageContext.request.contextPath}/after/role/get_all_menu.action" checkbox="true"></ul>
</div>
...
<script type="text/javascript">
    ...
    function add() {
        ...
        // alert(data);
        $.ajax({
            url: '${pageContext.request.contextPath}/after/role/add.action',
            dataType: 'json',
            type: 'post',
            data: data,
    ...
    function edit() {
        ...
        // alert(data);
        $.ajax({
            url: '${pageContext.request.contextPath}/after/role/edit.action',
            dataType: 'json',
            type: 'post',
            data: data,
            success: function (data) {
    ...
    function remove() {
        $.messager.confirm('信息提示', '确定要删除该记录？', function (result) {
            if (result) {
                ...
                $.ajax({
                    url: '${pageContext.request.contextPath}/after/role/delete.action',
                    dataType: 'json',
                    type: 'post',
                    data: {id: item.id},
                    success: function (data) {
    ...
    function openEdit() {
        ...
        $('#edit-dialog').dialog({
            ...
            onBeforeOpen: function () {
                //选择单选按钮的值
                $("input[name='available']").each(function () {
                    if ($(this).val() == item.available) {
    ...
    function isAdded(row, rows) {
        for (var k = 0; k < existAuthority.length; k++) {
            if (existAuthority[k].pid == row.id && haveParent(rows, row._parentId)) {
    ...
    function haveParent(rows, parentId) {
        for (var i = 0; i < rows.length; i++) {
            if (rows[i].id == parentId) {
                if (rows[i]._parentId != 0) return true;
            }
        }
        return false;
    }
    ...
    function selectAuthority(roleId) {
        $('#select-authority-dialog').dialog({
            ...
            buttons: [{
                ...
                handler: function () {
                    ...
                    if (ids != '') {
                        ...
                        $.ajax({
                            url: '${pageContext.request.contextPath}/after/role/add_role_permission.action',
                            type: "post",
                            data: {ids: ids, rid: roleId},
                            dataType: 'json',
                            success: function (data) {
                                if (data.type == 'success') {
            ...
            onBeforeOpen: function () {
                ...
                //首先获取该角色已经拥有的权限
                $.ajax({
                    url: '${pageContext.request.contextPath}/after/role/get_role_permission.action',
                    data: {id: roleId},
                    type: 'post',
                    checkbox:true,
                    dataType: 'json',
    ...
    $('#data-datagrid').datagrid({
        url: '${pageContext.request.contextPath}/after/role/list.action',
        rownumbers: true,
        singleSelect: true,
        pageSize: 20,
        pagination: true,
        ...
        columns: [[
            ...
            {
                field: 'available', title: '是否可用', width: 100, formatter: function (value, row, index) {
                    return row.available == 1 ? '<font color=blue>可用</font>' : '<font color=red>不可用</font>';
                    // alert(row.available);
                }
            },
