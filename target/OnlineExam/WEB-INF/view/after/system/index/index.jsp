...
<meta name="copyright" content="All Rights Reserved, Copyright (C) 2020, wadewhy, Ltd." />
<title>后台管理主页</title>
...
<div id="loading" style="position:absolute;z-index:1000;top:0px;left:0px;width:100%;height:100%;background:#FFFFFF;text-align :center;padding-top:6%;">
	...
	<div class="wu-header" data-options="region:'north',border:false,split:true">
    	<div class="wu-header-left">
        	<h1>【wadewhy】考试后台管理系统</h1>
        </div>
        <div class="wu-header-right">
        	<p><strong class="easyui-tooltip" title="0条未读消息">${user.roles[0].name}:${user.user.name}</strong>，欢迎您！</p>
            <p><a href="${pageContext.request.contextPath}/after/sys/index.action" target="_blank">网站首页</a>|<a href="http://wadewhy.xyz">支持论坛</a>|<a href="#">帮助中心</a>|<a href="${pageContext.request.contextPath}/after/sys/logout.action">安全退出</a></p>
        </div>
    </div>
    <!-- end of header -->
    <!-- begin of sidebar -->
	<div class="wu-sidebar" data-options="region:'west',split:true,border:true,title:'导航菜单'"> 
    	<div class="easyui-accordion" data-options="border:false,fit:true"> 
        	...
        	<div title="${topMenu.title }" data-options="iconCls:'${topMenu.icon }'" style="padding:5px;">
    			<ul class="easyui-tree wu-side-tree">
                	<c:forEach items="${secondMenuList }" var="secondMenu">

                		<c:if test="${secondMenu.pid == topMenu.id }">
                			<li iconCls="${secondMenu.icon}"><a href="javascript:void(0)" data-icon="${secondMenu.icon}" data-link="${pageContext.request.contextPath}${secondMenu.href}?_mid=${secondMenu.id}" iframe="1">${secondMenu.title }</a></li>
                    	</c:if>
                    </c:forEach>
                </ul>
            </div>
    ...
    <div class="wu-main" data-options="region:'center'" style="width:1500px;">
        <div id="wu-tabs" class="easyui-tabs" data-options="border:false,fit:true">  
            <div title="首页" data-options="href:'${pageContext.request.contextPath}/after/sys/welcome.action',closable:false,iconCls:'icon-tip',cls:'pd3'"></div>
        </div>
    </div>
    <!-- end of main --> 
    <!-- begin of footer -->
